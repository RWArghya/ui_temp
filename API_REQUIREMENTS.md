...
| Export Resume PDF / LaTeX | "Print / Save PDF" & "Export LaTeX (.tex)" | `/api/profile/resume/pdf` & `/api/profile/resume/latex` | POST / Client | `{ profileId, config }` | Binary PDF / Plaintext `.tex` file | Client-side `window.print()` & `latexGenerator.js` / `api_req/profile.md` |
